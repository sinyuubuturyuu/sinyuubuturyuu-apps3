(function () {
  ...
  const state = {
    ...
    settingsCollection: DEFAULT_COLLECTION,
    sort: {
  ...
  async function loadSettingsDocsFromFirestore() {
    const settingsDb = state.settingsDb || state.db;
    const settingsCollection = state.settingsCollection || state.collection;
    if (!settingsDb || !SETTINGS_DOC_IDS.length) {
      return [];
    }

    const settingsSnaps = await Promise.all(
      SETTINGS_DOC_IDS.map((docId) => settingsDb.collection(settingsCollection).doc(docId).get())
    );
  ...
  function pickPreferredDriverName(existingValue, nextValue) {
    const existing = normalizeDriverNameKey(existingValue);
    const next = normalizeDriverNameKey(nextValue);
    if (!existing) return next;
    if (!next) return existing;
    if (/\s/.test(existing) !== /\s/.test(next)) {
      return /\s/.test(next) ? next : existing;
    }
  ...
  function dedupeDriverOptions(values) {
    const unique = new Map();
    Array.from(values || []).forEach((value) => {
      const text = normalizeText(value);
      const compactKey = normalizeDriverNameCompactKey(text);
      if (!text || !compactKey) return;
      unique.set(compactKey, pickPreferredDriverName(unique.get(compactKey), text));
    });
    return Array.from(unique.values());
  }

  function resolveDriverOptionValue(values, currentValue) {
    const text = normalizeText(currentValue);
    if (!text) return "";
    const compactKey = normalizeDriverNameCompactKey(text);
    return (values || []).find((value) => normalizeDriverNameCompactKey(value) === compactKey) || "";
  }
  ...
  function refreshFilterSelectOptions() {
    ...
    const selectedDriver = resolveDriverOptionValue(driverOptions, currentDriver);
  ...
  function openEditModal(docId) {
    ...
    fillSelect(
      ui.editDriverName,
      state.fieldOptions ? state.fieldOptions.driverName : [],
      resolveDriverOptionValue(state.fieldOptions ? state.fieldOptions.driverName : [], current.driverName),
      "選択"
    );
  ...
  function applyFilters() {
    ...
    state.filteredRows = state.rows.filter((row) => {
      ...
      if (driver && normalizeDriverNameCompactKey(row.driverName) !== normalizeDriverNameCompactKey(driver)) return false;
  ...
  async function initFirebase() {
    ...
    const primaryApp = getOrCreateFirebaseApp(window.APP_FIREBASE_CONFIG, null);
    ...
    const directoryConfig = window.APP_FIREBASE_DIRECTORY_CONFIG;
    const directorySyncOptions = window.APP_FIREBASE_DIRECTORY_SYNC_OPTIONS || {};
    if (!directorySyncOptions.enabled || !directoryConfig) {
      return;
    }

    try {
      const directoryApp = getOrCreateFirebaseApp(
        directoryConfig,
